...
libnpmconfig_1.default.read().forEach((value, key) => {
    ...
    if (lodash_1.default.isString(pacoteKey)) {
        ...
        lodash_1.default.assign(npmConfig, pacoteKey(normalizedValue));
    }
    else {
        npmConfig[key.match(/^[a-z]/i) ? lodash_1.default.camelCase(key) : key] = normalizedValue;
    }
});
...
async function viewOne(packageName, field, currentVersion, options = {}) {
    const result = await (0, exports.viewManyMemoized)(packageName, [field], currentVersion, options);
...
function satisfiesNodeEngine(versionResult, nodeEngineVersion) {
    ...
    if (!minVersion)
        return true;
    const versionNodeEngine = lodash_1.default.get(versionResult, 'engines.node');
...
const newest = async (packageName, currentVersion, options = {}) => {
    ...
    const versionsSatisfyingNodeEngine = lodash_1.default.filter(result.versions, version => satisfiesNodeEngine(version, options.nodeEngineVersion))
        .map((o) => o.version);
